---
layout: post
title:  "The Matrix Reloaded: introduction to matrix calculus"
date:   2025-07-24 10:22:40 +0200
update_date:  2025-07-24 11:22:40 +0200
categories: foundations
---

[New illustration with a reference to matrix?]
...
Similarly to the habit of [writing everything in matrix notations](), the latter skill has helped me achieve 100-fold speedups of machine learning code quite a few times, in both academic and industrial projects. So I'd say that it's a rather neat trick to have up your sleave.

<details>
<summary>Disclaimer for math purists</summary>

The objective of this article is, as evidenced by its title, to provide a *gentle* introduction to the topic of matrix calculus as used in the field of machine learning.
...
Objective: step by step approach, so that everything in [TLDR]() makes perfect sense at the end of this article.
...
$$
f^\prime(x) = \frac{\partial f(x)}{\partial x} = 3x^2 -2
$$
...
*\*You may want to check [this article]() on matrix notations if this notation is unclear.*
...
We could even further generalize this to any scalar-valued function of a [tensor](), although this would be slightly out of the scope I initially planned for this article.
...
In the previous section, the output of the function $f$ was a single scalar. What if it's a vector instead? Let's consider functions on one variable only for now.
...
An example of such a function could be
$\mathbf{f}(x_1,x_2,x_3)=\begin{pmatrix}x_1 + x_2 \\ -x_3^2\end{pmatrix}$. Exercise: compute the Jacobian $\frac{\partial \mathbf{f}}{\partial \mathbf{x}}$.
...
<details>
<summary>Of the indices' order in the definition, the universe and everything</summary>
...
If you asked me this question before 9AM, my answer would probably have been *"Mrmpf that's the convention, I don't make the rules. Where's my coffee?"*. Fortunately, I am now more woken up and in a good mood, so: one way to see why it makes sense is to consider the function $\mathbf{f}(\mathbf{x}) = \mathbf{Ax}$, with $\mathbf{x} \in \mathbb{R}^N$, $\mathbf{A} \in \mathbb{R}^{M \times N}$ and thus $\mathbf{f}(\mathbf{x}) \in \mathbb{R}^M$.
...
With the definition $\left( \frac{\partial \mathbf{f}}{\partial \mathbf{x}}\right)_{n,m} = \frac{\partial f_{m}}{\partial x_n}$ however, we would get
$$
\frac{\partial \mathbf{Ax}}{\partial \mathbf{x}} = \mathbf{A}^\top
$$
which feels... awkward and not quite right. So, $\left( \frac{\partial \mathbf{f}}{\partial \mathbf{x}}\right)_{m,n} = \frac{\partial f_{m}}{\partial x_n}$ it is then!
...
Ok, we have covered most of the things we will usually need in practice. But I feel like stopping there would leave a bit of an itch, as some tempting generalizations were kept under the rug: what about derivatives of matrices with respect to vectors? And vectors w.r.t. matrices, matrices w.r.t. matrices, etc?
...
While being a tensor is not an issue per se (every mathematical object deserves respect, regardless of its rank), they are mostly out of the scope of this article. For a slightly more detailed albeit brief introduction to tensors, you can read [this article]().
...
*\*at least within a limited scope of (multi)linear algebra.*
...
And there it is!
Simply stating this pretty much sums up everything we have seen so far. Yet, I feel that limiting this article to this definition may not have been the most pedagogical approach imaginable.

## Practice time! Useful identities and exercises
...
This way, we can consolidate what we just learned, continue building intuition *and* in the process derive a few useful results. How great is that?
...
In general, it is good practice to always do this type of sanity check before jumping into calculations. It helps clarify the role of each entity, and trains your muscle memory to detect inconsistencies when [writing in matrix notations]().
...
so $\forall i, \left(\frac{\partial \mathbf{a}^\top\mathbf{x}}{\partial \mathbf{x}}\right)_i = \frac{\partial \mathbf{a}^\top\mathbf{x}}{\partial x_i} = \frac{\partial \sum_{k=1}^K a_k x_k}{\partial x_i} = \sum_{k=1}^K \frac{\partial a_k x_k}{\partial x_i}$
...
= \frac{\partial}{\partial \mathbf{x}} \left( \sum_{n=1}^N a_n b_n\right) \\
& = \sum_{n=1}^N \frac{\partial a_n b_n}{\partial \mathbf{x}} \\
...
Wow, I really appreciate your enthusiasm. I'll just continue to the next section for now as the clock is ticking and we need to move on, but I added a couple of exercises at the end in case you want to continue practicing.

## *Le plat de résistance*

OK, now that we're super familiar with matrix calculus, it's time for me to keep my promises, and show that finding a closed-form solution for least square linear regression is really not that difficult — in fact, with enough practice, you should be able to do this is your sleep*.
...
*(If the reason why the two sides of the equation above are equal is not clear, now would be a great time to read [this article](). Actually, any time would be a great time to read this article).*

### Solving least square linear regression
...
*JFC**, have you been paying attention *at all*? Finding analytical formulas for gradients is all we've been doing for the past 30 minutes!
...
First, a useful thing to notice: $||\mathbf{Xw} - \mathbf{y}||_2^2 = (\mathbf{Xw} - \mathbf{y})^\top(\mathbf{Xw} - \mathbf{y})$
...
In case you skipped the exercises above: well, first: boooh! You should do the exercises, it's very good practice. It is however possible to directly prove the result above directly from the definition, without using any the intermediate results we derived in the previous section, by using sum and index notations — as opposed to [matrix notations]().

However, as you're about to see, this gets pretty cumbersome pretty quickly, so I'd rather encourage you to make use of matrix notations and  intermediate results whenever possible.
...
Least square and pseudo-inverse
(pseudo-inverse: super useful)

Also PCA etc as exercises

## TL;DR

Below is a brief and slightly indigestible summary of the content covered in the article, for future reference or to check if you would benefit from reading it.
...
Summary of part on linear regression:
...
Use the fact that $\mathbf{A}^{-1}\mathbf{A} = \mathbf{I}_N$ and result from exercise 2.
</details>
...
$$
where $||\cdot||_F$ is the [Frobenius norm](https://en.wikipedia.org/wiki/Matrix_norm#Frobenius_norm) defined as
...
$$
\frac{\partial ||\mathbf{AXB+C}||_2^2}{\partial \mathbf{X}}
