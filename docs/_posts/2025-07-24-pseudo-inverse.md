...
What is the "point commun" between linear regression, 3D hand landmarks localization, 3D facial shape estimation and wrist bone estimation?

The solution to all these problems ended up being the use of a super powerful tool: pseudo-inverse. So I figured this is a topic that might deserve its own article.

Similarly to the articles on [matrix notations]() and [matrix calculus](), this post is another bastard child between ML and applied math, too basic and not rigorous enough to be fully math, and too generic and foundational to really be ML.

While I'm at it, I also generalize...
...
This system is nice enough to already be in triangular form, so it is easy to see that there is exactly one solution: $(x_1,x_2) = (4,2)$.
...
Even better yet, we can read the [article on matrix notation ]() and remember that it is usually a good idea to write everything in matrix form whenever possible, and rewrite the system as
...
With the system from Equation (1), we have
$$\mathbf{A} = \begin{pmatrix}
2 & 1 \\
1 & 0
...
Now, we can remember from linear algebra classes that this system has a unique solution if and only if the rows of $\mathbf{A}$ are linearly independant / the columns of $\mathbf{A}$ are linearly independant / $\mathbf{A}$ is inversible / $\text{det}(\mathbf{A}) \neq 0$ / yada yada yada,
in which case the solution is given by
$$
\mathbf{x} = \mathbf{A}^{-1}\mathbf{b}
$$
*(To obtain this, we simply left-multiply both sides of Equation (2) by $\mathbf{A}^{-1}$, such that $\mathbf{A}^{-1}\mathbf{A}$ cancels on the left of $\mathbf{x}$)*
...
We are looking for a vector $\mathbf{x} = (x_1, \dots, x_N)^\top$ which *satisfies* a number of constraint. Line 1 of Equation (X) states that the vector $\mathbf{x}$ we are looking for *must* be such that:

$$
a_{1,1}x_1 + \dots + a_{1,N}x_N = b_1
$$

Similarly, line 2 of Equation (X) states that $\mathbf{x}$ must satisfy
...
Wait... How can that make any sense, Yannick? You must have made a typo or something: this system obviously can't be solved anymore, as $x_1$ cannot be simultaneoulsy equal to 4 *and* to 6.
...
Intuitively, choosing $x_1=5$ seems closer to a solution than choosing $x_1 = 10^{10^{100}}$, since 5 is fairly close to both 4 and 6 (constraints of lines 2 and 3 in Equation (X)), whereas $10^{10^{100}}$ is not by any reasonable measure.
So, how could we formulate this idea in a more mathematical way?

Perhaps we could "loosen" our constraints a bit: instead of requiring that $\mathbf{x}$ must exactly satisfy each equality, we could merely ask that it is kind of close to satisfying each constraint. This would be analoguous to rewriting our system from Equation (3) as:
...
(Since we are considering "over-constrained" systems of equations in this section, we assume for now that $N > K$).
...
*(If the jump from Equation (3) to Equation (4) above looks somehow confusing to you, you may want to read this other [article on matrix notations](), which I consider to be a pre-requisite to all my blog posts including this one.)*
...
That is, we are trying to solve the following optimization problem:
...
This quickly leads to a pretty deep rabbit hole, but very briefly, the argument is the same as for linear regression: we assume that the *residues*, or the training errors in the case of linear regression, are independant and are normally distributed. In this case, using maximum likelihood to estimate the parameters of the model, which are our unknowns in this case, is equivalent to minimizing the squares of the errors.
...
Second, you may recognize that this problem is very similar to the optimization problem for linear regression from e.g. [this article](), namely:
$$
...
All of this may be nice conceptually but of very little practical use if we can't solve Equation (12). But as you may expect, I was not going to [rant] for multiple pages to conclude by *"This problem has no solution, too bad, all we just did was utterly useless"*.

So spoiler, alert:
<div class="myhighlight" markdown="1">
The problem does have a neat, analytical solution, which is given by

$$\mathbf{x} = (\mathbf{A}^\top\mathbf{A})^{-1}\mathbf{A}^\top\mathbf{b}$$

</div>

Getting to this result is not too difficult, and is a nice application of [matrix calculus]().
I am now going to briefly describe how we can get to this result. Feel free to skip this part if you're OK with just admitting that this is true.

First step: let's (re)clarify what we are looking for.

Although the expression $||\mathbf{Ax} - \mathbf{b}||_2^2$ contains matrices and vectors, the quantity itself is a scalar. We are trying to minimize this quantity with respect to $\mathbf{x} \in \mathbb{R}^K$, so the (loss) function we are interested in is:
$$
...
You may want to check out this [other post]() on matrix calculus. If you were already redirected to this post from [this post](), I promise this is the last post you will need to check before having the full explanation for why [solution to linear regression]
</details>
...
To prove this in a more rigorous way, in 1 dimension, we could compute the second order derivative and prove that $\frac{\partial^2 \mathcal{L}(x)}{\partial x^2} > 0$.
More generally, in $K$ dimensions, we could compute the Hessian matrix $\mathbf{H}$ defined by
$H_{i,j} = \frac{\partial^2 \mathcal{L}(\mathbf{x})}{\partial x_i \partial x_j}$ and show that is is positive definite, i.e. $\forall \mathbf{x} \in \mathbb{R}^K, \mathbf{x} \neq \mathbf{0} \implies \mathbf{x}^\top \mathbf{H} \mathbf{x} > 0$. But this is outside of the scope of this article.

For a similar overview of this argument which more explicitly relies on the linear regression formulation, you may check out [this article]().
